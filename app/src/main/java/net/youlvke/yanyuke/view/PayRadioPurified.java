...
public class PayRadioPurified extends RelativeLayout implements Checkable {
	...
	public PayRadioPurified(Context context, AttributeSet attrs) {
		...
		TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.PayRadioButton);
		
		Drawable d = array.getDrawable(R.styleable.PayRadioButton_radioPrb);
		if (d != null) {
			payChecked.setButtonDrawable(d);
		}
		
		String title = array.getString(R.styleable.PayRadioButton_titlePrb);
		if (title != null) {
			setTextTitle(title);
		}
		
		String str = array.getString(R.styleable.PayRadioButton_descPrb);
		if (str != null) {
			setTextDesc(str);
		}
		
		Drawable logo = array.getDrawable(R.styleable.PayRadioButton_logoPrb);
		if (logo != null) {
			setDrawableLogo(logo);
		}
		
		boolean checked = array.getBoolean(R.styleable.PayRadioButton_checkedPrb, false);

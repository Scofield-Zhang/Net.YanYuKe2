...
import static net.youlvke.yanyuke.R.id.et_consinee;
 ...
 * 管理收货地址
 */
public class MotifyAddressActivity extends BaseActivity {
    // 省数据集合
    ...
    private AddressInfo.DataBean dataBeanCurrent;
    ...
    protected void initView() {
        ...
        etConsinee = (EditText) findViewById(et_consinee);
    ...
    protected void initData() {
        ...
        btnSave.setText("确定提交");
        // 初始化Json对象
        initJsonData();
        // 初始化Json数据
        initJsonDatas();
        dataBeanCurrent = (AddressInfo.DataBean) getIntent().getSerializableExtra("currentBean");
        setData();
    ...
    private void setData() {
        etConsinee.setText(dataBeanCurrent.getContactName());
        tvTelNumber.setText(dataBeanCurrent.getContactPhone());
        tvChoice.setText(dataBeanCurrent.getProvince() + dataBeanCurrent.getCity() + dataBeanCurrent.getRegion());
        etDetailAddress.setText(dataBeanCurrent.getAddressDetail());
        etConsinee.setText(dataBeanCurrent.getContactName());
        isDefualt.setSelected(true);
    }
    ...
    protected void onInnerClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_save:// 提交
                ...
                String address = dataBeanCurrent.getProvince()+dataBeanCurrent.getCity()+dataBeanCurrent.getRegion();

                if (Cansinee == null && dataBeanCurrent.getContactName().equals(Cansinee) &&
                        telNumber == null && dataBeanCurrent.getContactPhone().equals(telNumber) &&
                        mProvince == null && dataBeanCurrent.getAddressDetail().equals(detailAddress)
                        && detailAddress == null) {
                    return;
                }

                if ( dataBeanCurrent.getContactName().equals(Cansinee) && tvChoice.getText().equals(address)
                        &&dataBeanCurrent.getContactPhone().equals(telNumber) && dataBeanCurrent.getAddressDetail().equals(detailAddress)) {
                    finish();
                }
                if (mProvince == null ){
                    mProvince = dataBeanCurrent.getProvince();
                    mCity= dataBeanCurrent.getAddressDetail();
                    mArea = dataBeanCurrent.getRegion();
                }
            ...
            IRequest.post(this, Constants.BASEURL_YYKUSER, createGetAddress(addressDetail, contactName, contactPhone,
                ...
                public void onSuccess(Response<String> response) {
                    ...
                    try {
                        ...
                        if (code == 1) {
                            ToastUtils.showToast(MotifyAddressActivity.this, "修改成功");
                            finish();
                        } else if (code == 2) {
                            ToastUtils.showToast(MotifyAddressActivity.this, "添加失败");
                        }
    ...
    private Map createGetAddress(String addressDetail, String contactName, String contactPhone,
        ...
        params.put("addressId", dataBeanCurrent.getAddressId());
        return params;
    }
